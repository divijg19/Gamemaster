...
use crate::commands::games::deck::{Deck, Rank};
...
impl Game for PokerGame {
    ...
    async fn handle_interaction(
        ...
        match self.phase {
            ...
            _ => {
                self.send_ephemeral_response(ctx, interaction, "It's not time for actions.")
                    .await;
                GameUpdate::NoOp
            }
        }
...
impl PokerGame {
    ...
    pub fn deal_new_round(&mut self) {
        ...
        self.dealer_hand = self.deck.deal(5).unwrap();
        self.dealer_rank = Some(evaluate_hand(&self.dealer_hand));

        for player in self.players.iter_mut() {
            player.status = PlayerStatus::Playing;
            player.hand = self.deck.deal(5).unwrap();
    ...
    pub fn calculate_payouts(&self) -> (String, Vec<GamePayout>) {
        let dealer_rank = self.dealer_rank.unwrap();
        ...
        for player in &self.players {
            ...
            let result_str = match player.status {
                ...
                PlayerStatus::Playing => {
                    let player_rank = player.hand_rank.unwrap();
                    if !dealer_qualifies {
                        total_winnings += player.ante_bet; // Win 1:1 on Ante
                        // Play bet is a push
                        format!("**Wins!** Dealer did not qualify. (Ante pays 1:1)")
                    } else if player_rank > dealer_rank {
                        total_winnings += player.ante_bet; // Win 1:1 on Ante
                        total_winnings += player.play_bet; // TODO: Implement bonus payout table for strong hands
