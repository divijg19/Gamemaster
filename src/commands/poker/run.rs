...
// ... (You can add a `run_prefix` function here if desired, following the same pattern)
...
fn spawn_lobby_timeout_handler(
    ...
    tokio::spawn(async move {
        ...
        if let Some(game_box) = manager.get_game_mut(&game_msg.id) {
            if let Some(poker_game) = game_box.as_any().downcast_ref::<PokerGame>() {
                if poker_game.phase == super::state::GamePhase::WaitingForPlayers {
