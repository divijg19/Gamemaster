...
pub fn register() -> CreateCommand {
    CreateCommand::new("bond")
        ...
        .add_option(serenity::builder::CreateCommandOption::new(
            ...
            "create",
            "Create a new bond (host + equipped)",
        ))
        .add_option(serenity::builder::CreateCommandOption::new(
            serenity::model::application::CommandOptionType::SubCommand,
            "status",
            "View active bond contributions",
        ))
}
...
pub async fn run_slash(ctx: &Context, interaction: &CommandInteraction) {
    ...
    let Some(app_state) = data.get::<AppState>().cloned() else {
        return;
    };
    ...
    match sub {
        "status" => {
            match crate::database::units::list_bond_contributions(&pool, interaction.user.id).await
            {
                Ok(list) if !list.is_empty() => {
                    ...
                    for b in list {
                        by_host.entry(b.host_player_unit_id).or_default().push(b);
                    }
                    ...
                    for (host, bonds) in by_host.iter() {
                        ...
                        for b in bonds {
                            host_totals.0 += b.bonus_attack;
                            host_totals.1 += b.bonus_defense;
                            host_totals.2 += b.bonus_health;
                            grand_totals.0 += b.bonus_attack;
                            grand_totals.1 += b.bonus_defense;
                            grand_totals.2 += b.bonus_health;
                            desc.push_str(&format!(
                                " • [#{}] {} {} (+{} Atk / +{} Def / +{} HP) [eq_id:{}]\n",
                                b.bond_id,
                                rarity_icon(b.rarity),
                                b.equipped_name,
                                b.bonus_attack,
                                b.bonus_defense,
                                b.bonus_health,
                                b.equipped_player_unit_id
                            ));
                        }
                        desc.push_str(&format!(
                            "   └─ Host Total: +{} / +{} / +{}\n\n",
                    ...
                    desc.push_str(&format!(
                        "**Grand Total Bonuses:** +{} Atk / +{} Def / +{} HP",
                        grand_totals.0, grand_totals.1, grand_totals.2
                    ));
                    ...
                    interaction
                        .edit_response(
                            &ctx.http,
                            EditInteractionResponse::new().embed(embed).content(" "),
                        )
                        .await
                        .ok();
                }
                    ...
                    interaction
                        .edit_response(
                            ...
                            EditInteractionResponse::new()
                                .content("Failed to load bond contributions."),
                        )
                        .await
                        .ok();
                }
        ...
        _ => {
            ...
            let units = match database::units::get_player_units(&pool, interaction.user.id).await {
                Ok(v) => v,
                Err(_) => {
                    interaction
                        .edit_response(
                            &ctx.http,
                            EditInteractionResponse::new().content("Could not load units."),
                        )
                        .await
                        .ok();
                    return;
            ...
            let (embed, components) =
                crate::commands::bond::ui::create_bond_select(&hosts, &candidates);
            interaction
                .edit_response(
                    &ctx.http,
                    EditInteractionResponse::new()
...
pub async fn run_prefix(ctx: &Context, msg: &Message, _args: Vec<&str>) {
    ...
    let units = match database::units::get_player_units(&pool, msg.author.id).await {
        Ok(v) => v,
        Err(_) => {
            msg.reply(&ctx.http, "Could not load units.").await.ok();
            return;
        }
    };
    use crate::database::models::UnitRarity;
